...
module.exports = class Moderatie extends command {
  constructor(client) {
    super(client, {
      name: "tempban",
      description: "Ban iemand tijdelijk!",
      category: "Moderatie",
      subCommands: ["b"]
    }, {
      permissions: {
  ...
  async run(message, args) {
    ...
    if (!args[1] || isNaN(Number(args[1]))) return message.error(`Tijd is niet (correct) opgegeven. De tijd moet in dagen.\nVoorbeeld: \`${message.author.settings.prefix}tempban ${args[0]} 10 <reden>\` - Bant iemand voor 10 dagen.`)
    ...
    collector.on("collect", async (reaction) => {
      ...
      let bannedUsers = await this.client.db.get("tempbans");
      bannedUsers.push({
        user: member.id,
        unbanDate: Date.now() + 1000*60*60*24*time,
        guild: message.guild.id
      });
      await this.client.db.set("tempbans", bannedUsers);

      await message.guild.channels.find(c => c.name === this.client.config.modlogs).send(message.embed()
        .setTitle("Ban")
        .setDescription(`<@${member.id}> is tijdelijk verbannen door <@${message.author.id}> in <#${message.channel.id}>.`)
        .addField("**Reden**", reden)
        .addField("**Tijd**", `${time} dagen.`)
        .setColor("#ff0000")
      ...
      try {
        await member.user.send(message.embed()
          .setTitle("Ban")
          .setDescription(`Je bent tijdelijk verbannen door <@${message.author.id}> in **${message.guild.name}**. Reden: **${reden}**`)
          .addField("**Tijd**", `${time} dagen.`)
          .setColor("#ff0000")
